...
export async function fulfillPurchase(
  ...
  const consumed =
    (call.data as { consumed?: unknown } | null)?.consumed === true;
...
async function picoCall(
  ...
  if (!response.ok) {
    throw fault(
      'PICO.HTTP_FAILED',
      `PICO returned a ${response.status} status.`,
      {
  ...
  if (typeof record?.code !== 'number') {
    throw fault(
      'PICO.MALFORMED_RESPONSE',
      'The PICO response envelope has no code.',
      {
        retryable: true,
      },
    );
...
function occurredAtOf(grantTime: number): string {
  const milliseconds =
    grantTime > 99_999_999_999 ? grantTime : grantTime * 1000;
...
function narrowProof(proof: unknown): PicoProof {
  if (proof !== null && typeof proof === 'object') {
    ...
    if (
      ...
      return {
        userAccessToken: record.userAccessToken,
  ...
  throw fault(
    'PICO.MALFORMED_PROOF',
    'A PICO proof must carry userAccessToken, userId, and sku.',
    {
      detail: { proof },
    },
  );
